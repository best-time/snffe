...
var DragLoading = function (el, option) {
    ...
    params.trigger.on({
        touchstart: function (event) {
            ...
            // 垂直方向
            data.nowY = data.posY = events.pageY;
        ...
        touchmove: function (event) {
            if (data.touching !== true) return

            var events = event.touches[0] || event;

            // 垂直位移
            data.nowY = events.pageY;
            var distanceY = data.nowY - data.posY; // 当前位置 - touchStart Y轴位置 = 移动的距离
            ...
            if (elWindow.scrollTop() == 0 && distanceY > 0) {
                // other browser, 已经滚动到头，阻止默认滚动行为，例如iPhone Chrome的下拉加载
            ...
            if (data.markY > 0 && el.data('loading') != true) {
                ...
                // }
                valHeight = moveY < 0 ? 0 : Math.min(moveY, maxY)

                // borderBottomWidth增加阻力
                var overflowHeight = 0 - valHeight / 2 + Math.max(0, moveY - maxY),
                    borderBottomWidth = overflowHeight;
                if (overflowHeight > 0) {
