...
(function () {
    ...
    var AlloyTouch = function (option) {
        ...
        this.sensitivity = this._getValue(option.sensitivity, 1);   // 默认1
        this.moveFactor = this._getValue(option.moveFactor, 1);     // 默认1
        this.factor = this._getValue(option.factor, 1);             // 默认1
        this.outFactor = this._getValue(option.outFactor, 0.3);     // 默认 0.3
        this.min = option.min;
        this.max = option.max;
        ...
        this.lockDirection = this._getValue(option.lockDirection, true);    // 默认 true
        ...
        _move: function (evt) {
            if (this.isTouchStart) {
                ...
                if (this._firstTouchMove && this.lockDirection) {
                    ...
                    if (dDis > 0 && this.vertical) { // x轴 移动差 > y轴移动差  && 竖直方向
                        this._preventMove = true;
                    } else if (dDis < 0 && !this.vertical) { // x轴 移动差 < y轴移动差  && 水平方向
                        this._preventMove = true;
                    }
                ...
                if(!this._preventMove) {
                    ...
                    if (this.hasMax && this.target[this.property] > this.max && d > 0) { // 滑到顶部
                        f = this.outFactor;
                    } else if (this.hasMin && this.target[this.property] < this.min && d < 0) { // 滑到底部
                        f = this.outFactor;
                    }
                    d *= f;
                    this.preX = currentX;
                    this.preY = currentY;
                    if (!this.fixed) {console.log(this.target[this.property])
