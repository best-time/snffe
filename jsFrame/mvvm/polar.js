function Directive(el, polar, attr, elementValue) {
  ...
  this.el[this.attr] =
      this.elementValue = elementValue; //初始化
}
  ...
  _compile(root) {
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      if (matches) {
        ...
        if (splitTextNodes[0]) {
            var txt = document.createTextNode(splitTextNodes[0])
          node.append(txt);
        }
        ...
        for (let ii = 0; ii < newMatches.length; ii++) {
          ...
          var tmp2 = new Directive(el, this, "nodeValue", this.data[newMatches[ii]])
          //对数据和dom进行绑定
          //let returnCode =
              this._data.__bindings[newMatches[ii]]
              ?
              this._data.__bindings[newMatches[ii]].push(
                tmp2
              )
              :
              (this._data.__bindings[newMatches[ii]] = [
      ...
      // 有 p-model 属性, 且是 input  或者 textarea  
      if (
        (node.hasAttribute("p-model") &&
          node.tagName.toLocaleUpperCase() == "INPUT") ||
        node.tagName.toLocaleUpperCase() == "TEXTAREA"
      ) { 
